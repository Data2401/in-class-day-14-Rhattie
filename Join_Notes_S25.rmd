...
I also highly suggest you read through Chapter 11 of our textbook, or Chapter 13 of R for Data Science: <https://r4ds.had.co.nz/relational-data.html> or this lecture from Jenny Bryan: <https://stat545.com/join-cheatsheet.html>
...
inner_join(members, 
           band_instruments,join_by(player_name == name))
```
...
```{r}
patients %>% filter(name == 'bob')
#left_join(visits,patients ) %>% left_join(doctors) %>% filter(name == 'bob')%>% select(name,docter_name, specialty, everything())
```
Find patients that haven't visited the hospital
```{r}
anti_join(visits,doctors)
