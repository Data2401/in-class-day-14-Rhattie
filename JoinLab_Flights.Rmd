...
```{r}
avg_arrival_delays <- flights %>% group_by(dest) %>%summarise(avg_arr_delay = mean(arr_delay, na.rm = TRUE)) %>% arrange(desc(avg_arr_delay))         
avg_arrival_delays
```
...
```{r}
arrival_with_airports <- avg_arrival_delays %>%
  left_join(airports, by = c("dest" = "faa"))

arrival_with_airports
```
...
```{r}
avg_departure_delays <- flights %>%
  group_by(carrier) %>% 
  summarise(avg_dep_delay = mean(dep_delay, na.rm = TRUE)) %>% 
  arrange(desc(avg_dep_delay)) 


avg_departure_delays_with_names <- avg_departure_delays %>%
...
```{r}
smallest_delay_airport <- arrival_with_airports %>%
  arrange(avg_arr_delay) %>% 
  slice(1) %>% 
  select(name, avg_arr_delay) 

smallest_delay_airport
